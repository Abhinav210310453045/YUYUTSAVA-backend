...
class FakeUsageStore:
    ...
    async def list(self, *, task_id=None, since=None, limit=200):
        return [r for r in self.rows if since is None or r.ts >= since][:limit]

    async def aggregate(self, *, since=None, group_by=None):
        return []


def row(thread="T", *, tin=1000, tout=10, cost=0.001, model="gemini-3.5-flash", ts=None):
    return UsageRow(
